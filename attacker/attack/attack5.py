...
# async def enum():
#     client = Client(url="opc.tcp://localhost:26544")
#     try:
...
async def write():
    ...
    try:
        ...
        node_to_write = await root.get_child(["0:Objects", "1:EnvironmentalSensors", "1:Temperature"])
        print("Writing to node:", node_to_write)
        
        value_to_write = 20.0
        await node_to_write.set_value(value_to_write)
        
        node_to_write = await root.get_child(["0:Objects", "1:EnvironmentalSensors", "1:Humidity"])
        print("Writing to node:", node_to_write)
